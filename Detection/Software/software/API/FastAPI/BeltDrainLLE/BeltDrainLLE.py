...
class SettlingSettings(BaseModel):
    scanInterval: int = Field(20, title = "The interval for scanning", description = "The interval for scanning the funnel in seconds", gt = 0,example = 4)#300
    settlingTime: int = Field(120, title = "", description = "The time lapse for settling in seconds", gt = 0,example = 16)#3600
...
class DataDrain(BaseModel):
    mlToDrainLower: Union[float,None] = Field(default = None, title = "A number of ml lower phase", description = "The number of ml to drain", example = 100.0) 
    ...
    mlToDrainUpper: Union[float,None] = Field(default = None, title = "A number of ml upper phase", description = "The number of ml to drain", example = 100.0) 
...
class LiquidExtractor:
    ...
    async def runDrainExp(self,lType):
        ...
        if(interfaceFound):
            ...
            self.results.interfaceResult = DataInterface(interfaceFound = interfaceFound,interfacePosition = interfacePosition,lastScanTime=datetime.now())
...
def calculateVolumeFromPosition(position:float = None,initialVolume:float = 300, tubingVolume:float = 0,r:float =0.0455,funnelAngle:float = 0.296706):
    if position == None:
